...
describe("DeepSeek adapter", () => {
  ...
  it("proxy-routed CLI launch targets the default proxy and injects the proxy-local key, not DeepSeek's own key", () => {
  ...
  it("a provider-specific failure: missing proxy key surfaces as ProviderAuthError at launch time, not a generic error", () => {
    delete process.env.CONTINUUM_TENCENT_PROXY_USER_KEY;
    const adapter = createProviderAdapter(deepseekProfile);
    expect(() => adapter.buildCliLaunchPlan({ workingDir: "C:\\fake\\project" })).toThrowError(
      ProviderAuthError,
    );
  });
  ...
  it("env/auth isolation: DeepSeek's launch never reads Claude's ANTHROPIC_API_KEY, and vice versa", () => {
    ...
    // DeepSeek's proxy-routed launch has its own secret requirement and must
    // still fail even though an unrelated provider's key is present in env —
    // proves no accidental cross-provider fallback.
    expect(() => deepseekAdapter.buildCliLaunchPlan({ workingDir: "C:\\fake" })).toThrowError(
      ProviderAuthError,
    );

    process.env.CONTINUUM_TENCENT_PROXY_USER_KEY = "sk-mem-deepseek-only";
    delete process.env.ANTHROPIC_API_KEY;
    const claudeAdapter = createProviderAdapter(claudeProfile);
    // Claude's direct-call auth must still fail even though DeepSeek's proxy
    // key is present — proves the reverse direction too.
